...
So a V3 emulation is perfectly feasible, but it means adding a
`#ifdef VELXIO_EMULATION` shim to `game-prototype.ino` — the sketch that documents
what V4 inherited. This is the 2019 board most worth emulating (it has a real
game to assert on: the same two secret codes V4 and V5 use), so it is the one to
start with if the answer turns out to be yes. That call is open in
...
diagrammed ([../images/wiring-v3.png](../images/wiring-v3.png)) and its firmware
