"""Generate the three Lemon Piano wiring diagrams — declaratively.
...
**wirewright** engine (../eda-wirewright/), which DRC-validates every diagram
before saving. The engine lives in its own repo now; this project just consumes
it (via the src path shim below — no install needed, only Pillow).
